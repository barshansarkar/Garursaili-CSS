...
export function splitPrefixesFromRaw(token: string) {
  if (!token) return { prefixes: [], raw: token };
...
export function isBreakpoint(prefix: string) {
  const bps = (configDefault as any)?.breakpoints || {};
  return !!bps[prefix];
}

// this function is where my soul went to die
// someone wrote "sm:md:lg:hover:dark:group-hover:[&_*]:first" and expected it to work
export function applyVariantSelectors(baseSelector: string, variantParts: string[]) {
  ...
  for (const v of variantParts) {
    ...
    // the classics
    if (v === "hover") selector = `${selector}:hover`;
    else if (v === "focus") selector = `${selector}:focus`;
    else if (v === "active") selector = `${selector}:active`;
    else if (v === "visited") selector = `${selector}:visited`;
    else if (v === "focus-visible") selector = `${selector}:focus-visible`;

    else if (v === "first") selector = `${selector}:first-child`;
    else if (v === "last") selector = `${selector}:last-child`;
    else if (v === "odd") selector = `${selector}:nth-child(odd)`;
    else if (v === "even") selector = `${selector}:nth-child(even)`;

    else if (v === "group-hover") selector = `.group:hover ${selector}`;
    else if (v === "group-focus") selector = `.group:focus ${selector}`;

    else if (v === "peer-hover") selector = `.peer:hover ~ ${selector}`;
    else if (v === "peer-focus") selector = `.peer:focus ~ ${selector}`;

    else if (v === "dark") {
      if ((configDefault as any).darkMode === "media") {
        // do nothing here, builder will wrap it later
        selector = `${selector}`;
      } else {
        const darkSel = (configDefault as any).darkSelector || ".dark";
...
export function wrapWithBreakpoints(rule: string, bps: string[]) {
  const bpsConfig = (configDefault as any)?.breakpoints || {};
  let out = rule;
