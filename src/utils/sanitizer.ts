...
export function normalizeColorToken(val: string): string | null {
if (!val) return null;
const raw = val.trim();

// just let css functions live their life pls
if (/^var\(/.test(raw) || /linear-gradient|radial-gradient|conic-gradient|rgba?\(|hsla?\(/i.test(raw)) return raw;
...
if (hexOnly.test(raw)) return `#${raw}`;

if (/^#([0-9a-fA-F]{3,4}|[0-9a-fA-F]{6}|[0-9a-fA-F]{8})$/.test(raw)) return raw;

// keywords that are safe i guess
if (/^(transparent|currentColor|inherit|initial|revert)$/.test(raw)) return raw;
...
export function ensureUnitForLength(val: string): string {
if (!val) return val;
const s = val.trim();

if (s === "0") return "0";
if (/^calc\(/.test(s)) return s;

// already has unit, good for you
if (/^-?\d+(\.\d+)?(px|rem|em|vw|vh|ch|%)$/.test(s)) return s;
...
export function ensureTimeUnit(val: string): string {
const s = val.trim();
if (/^\d+$/.test(s)) return `${s}ms`;
if (/^\d+ms$/.test(s) || /^\d+s$/.test(s)) return s;
return s; // probably fine
}

export function normalizeTokenValueForProperty(prop: string, value: string): string | null {
...
if (b && !isNaN(a) && !isNaN(b)) return `${(a / b) * 100}%`;
  }
...
if (/^-?\d+(\.\d+)?$/.test(v)) {
const n = Number(v);
if (Math.abs(n) > 3) return `${n / 100}`;
return `${n}`;
    }
return v;
  }

if (/duration|delay/.test(prop)) return ensureTimeUnit(v);
