...
// copyright by barshan sarkar
// this cache was made in barshansarkarlabs
// this is v-1.0.0
// -------- TYPES (pls don’t ask me why they’re here it's 3am) ---------
...
const DEFAULT_CACHE: GarurCache = {
...
// ------------------------------------------------------
function getWriteCachePath(): string {
...
// ------------------------------------------------------
export function loadCache(): GarurCache {
...
// ------------------------------------------------------
export function saveCache(cache: GarurCache) {
  ...
  try {
      ...
      if (fs.existsSync(dest)) {
        try { fs.copyFileSync(dest, bak) } catch {}
      }
...
// ------------------------------------------------------
function validateCache(cache: any): GarurCache {

  const out: GarurCache = {
      ...
      if (typeof raw === "string") {
        ...
        out.files[norm] = {
          ...
          classes: Array.isArray(d.classes)
            ? d.classes.filter((x: any) => typeof x === "string")
            : [],
  ...
  if (v < 3) {
    if (raw.files) {
      for (const d of Object.values(raw.files)) {
        const dd = d as any
        if (!dd.timestamp) dd.timestamp = Date.now()
      }
    }
...
// ------------------------------------------------------
export function hasFileChanged(cache: GarurCache, filePath: string, content?: string) {
...
// ------------------------------------------------------
export function updateFileHash(cache: GarurCache, filePath: string, content: string) {
...
// ------------------------------------------------------
export function addFileClasses(cache: GarurCache, filePath: string, classes: string[]) {
...
// ------------------------------------------------------
export function removeFileFromCache(cache: GarurCache, filePath: string) {

  const p = normalizePathForCache(filePath)

  if (!cache.files[p]) return
...
// ------------------------------------------------------
export function getAllReferencedClasses(cache: GarurCache) {

  const s = new Set<string>()

  for (const f of Object.values(cache.files)) {
    for (const c of f.classes) s.add(c)
  }
...
export const getAllCachedClasses = (cache: GarurCache) => Object.keys(cache.classes)
export const getCachedFiles      = (cache: GarurCache) => Object.keys(cache.files)
export const hasClass            = (cache: GarurCache, cls: string) => cls in cache.classes
...
// ------------------------------------------------------
export function setClass(cache: GarurCache, className: string, css: string) {
  if (className && css) {
...
// ------------------------------------------------------
export function removeClass(cache: GarurCache, className: string) {
...
// ------------------------------------------------------
export function clearCache(): GarurCache {
...
// ------------------------------------------------------
export function getCacheStats(cache: GarurCache) {
...
// ------------------------------------------------------
export function pruneCache(cache: GarurCache, currentFiles?: string[]) {

  let pruned = 0
  const keep: Record<string, CachedFile> = {}

  const current = new Set<string>()
  if (Array.isArray(currentFiles)) {
    for (const f of currentFiles) current.add(normalizePathForCache(f))
  }
  ...
  const refs = getAllReferencedClasses(cache)
  const clsOut: Record<string, string> = {}

  for (const [cls, css] of Object.entries(cache.classes)) {
    if (refs.has(cls)) clsOut[cls] = css
    else pruned++
  }
...
// outputs json or csv
// ------------------------------------------------------
export function exportCache(cache: GarurCache, format: "json" | "csv" = "json") {
