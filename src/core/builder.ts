...
import {
  splitPrefixesFromRaw,
  applyVariantSelectors,
  wrapWithBreakpoints,
  isBreakpoint
} from "../utils/variants";
...
const CONFIG_BREAKPOINTS = (configDefault as any).breakpoints || {};
const BREAKPOINT_SET = new Set(Object.keys(CONFIG_BREAKPOINTS));
const DARK_MODE = (configDefault as any).darkMode;
const IMPORTANT_MODE = (configDefault as any).important === true;

// ==================== ENHANCED CACHES ====================
const buildCache = new Map<string, string | null>();
...
const prefixCache = new Map<string, { prefixes: string[], raw: string }>();
...
const colorPatternCache = new Map<string, { prefixes: string[], raw: string }>();
const MAX_COLOR_PATTERN_CACHE = 2000;

// ==================== ENHANCED ESCAPING ====================
export function escapeClassName(cls: string): string {
  if (!cls) return cls;
  ...
  if (escapeCache.size >= MAX_ESCAPE_CACHE) {
    ...
    if (firstKey) escapeCache.delete(firstKey);
  }
...
function cachedSplitPrefixesFromRaw(cls: string): { prefixes: string[], raw: string } {
  ...
  let result: { prefixes: string[], raw: string };
  ...
  if (isColorUtility) {
    ...
    if (colorCached) {
      ...
      if (colorPatternCache.size >= MAX_COLOR_PATTERN_CACHE) {
        ...
        if (firstKey) colorPatternCache.delete(firstKey);
      }
...
function cachedApplyVariantsToSelector(baseSelector: string, variantParts: string[]): string {
...
// ==================== ENHANCED PARSING ====================
function cachedParse(raw: string): any {
  ...
  try {
    ...
    const isColorLike = raw.includes('-') && (
      raw.startsWith('text-') ||
      raw.startsWith('bg-') ||
...
// ==================== ENHANCED HANDLING ====================
function cachedHandle(token: any): any {
  ...
  if (handlerCache.size >= MAX_HANDLER_CACHE) {
    ...
    if (firstKey) handlerCache.delete(firstKey);
  }
...
function applyImportantToDecl(decl: string, important: boolean): string {
  if (!important || !decl || IMPORTANT_MODE) return decl;
...
function wrapWithBps(rule: string, bps: string[]): string {
  if (bps.length === 0) return rule;
...
export function build(cls: string, inline: boolean = false): string | null {
...
function buildFromParts(
  ...
  let supportsQuery: string | undefined;
  let atRule: string | undefined;
  ...
  if (extra) {
    ...
    if (mediaQuery) extraWrapped = `@media ${mediaQuery} { ${extraWrapped} }`;
    if (containerQuery) extraWrapped = `@container ${containerQuery} { ${extraWrapped} }`;
    if (supportsQuery) extraWrapped = `@supports ${supportsQuery} { ${extraWrapped} }`;
    if (atRule) extraWrapped = `${atRule} { ${extraWrapped} }`;
...
export function warmupCache(commonClasses: string[] = [
...
// ==================== DEBUG UTILITIES ====================
export function debugBuild(cls: string): any {
  const { prefixes, raw } = cachedSplitPrefixesFromRaw(cls);
  const token = cachedParse(raw);
  const result = cachedHandle(token);
...
export function validateUtilities(utilities: string[]): { valid: string[], invalid: string[] } {
  const valid: string[] = [];
...
function fixColorUtilityParsing(raw: string): string {
