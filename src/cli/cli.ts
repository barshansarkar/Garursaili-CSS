...
let ora: ((options?: any) => any) | null = null;
...
const fileClasses = new Set<string>();
  Object.values(cache.files || {}).forEach((fileData: any) => {
if (fileData?.classes) {
      fileData.classes.forEach((cls: string) => fileClasses.add(cls));
    }
  });
  console.log("Classes in files: ", fileClasses.size);
const orphanedClasses = Object.keys(cache.classes || {}).filter(
...
let created = 0;
if (createConfigFile()) created++;
if (createPluginDemo()) created++;
if (createExampleHTML()) created++;
if (created > 0) {
...
const outputFile =
    args.output || args.o || path.resolve(process.cwd(), "dist/garur.css");
...
debugCache(before);
if (!ora) {
  const oraPkg = require("ora");  // Fallback to require for CJS interop
...
const format = (n: number) => n.toLocaleString();
  ...
  console.log(
`${pc.magenta("CSS size")} ${pc.bold(
      (stats.cssBytes / 1024).toFixed(2) + " KB"
    )}`
  );
