...
async function main() {
  ...
  try {
    ...
    const absolutePatterns = buildPatterns.map(p =>
  ...
  async function flushChanges() {
    if (isBuilding) {
      if (process.env.DEBUG) console.log(pc.yellow("Build already in progress, queuing..."));
      return;
    }
  ...
  watcher
    .on("add", (filePath) => {
      ...
      if (process.env.DEBUG) console.log(pc.gray(`+ ${path.relative(process.cwd(), fullPath)}`));
