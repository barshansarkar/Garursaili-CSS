...
function extractClassesFromSources(sources: string) {
  ...
  while ((m = classRe.exec(sources))) {
    ...
    for (const tok of raw.split(/\s+/).filter(Boolean)) {
      ...
      if (match) {
        set.add(match[1]);
        for (const inner of match[2].split(/\s+/).filter(Boolean)) set.add(inner);
      } else set.add(tok);
    }
  }
  return set;
...
async function main() {
  ...
  const safelist = (argv.safelist || "").split(",").map((s: string) => s.trim()).filter(Boolean);
  ...
  for (const s of safelist) used.add(s);
  ...
  for (const block of blocks) {
    ...
    for (const cls of Array.from(used)) {
      ...
      if (regex.test(sel)) { keep = true; break; }
    }
    // Also preserve @keyframes, :root variables and global utility extras
    if (!keep && (/^@keyframes/.test(block) || /^:root/.test(sel) || /--garur-/.test(block))) keep = true;
      ...
      if (/^@media/.test(block)) {
        ...
        for (const ib of innerBlocks) {
          ...
          for (const cls of Array.from(used)) {
            ...
            if (r.test(s)) { innerKeep = true; break; }
          }
          if (innerKeep) break;
        }
        if (innerKeep) kept.push(block);
      }
    }
  }
