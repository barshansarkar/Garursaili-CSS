// src/cli.ts
// ok its 3:15 am why am i still awake fixing this cli thing
// barshan sarkar here, indian css framework hero or whatever
...
const funMessages = [
    ...
    "This CSS is smaller than your average React bundle.",
];
...
// these functions are new and i regret adding them at 2am
function createConfigFile() {
    const configPath = path.resolve(process.cwd(), "garur.config.js");
    if (fs.existsSync(configPath)) {
        console.log(pc.yellow(`⚠️ Config already exists at ${configPath}`));
        return false;
    }
...
function createPluginDemo() {
    ...
    if (fs.existsSync(pluginPath)) {
        console.log(pc.yellow(`⚠️ Plugin already exists at ${pluginPath}`));
        return false;
    }
...
function createExampleHTML() {
    ...
    if (fs.existsSync(htmlPath)) {
        console.log(pc.yellow(`⚠️ Example already exists at ${htmlPath}`));
        return false;
    }
...
<body>
  <div class="container mx-auto">
    ...
    <!-- Responsive Demo -->
    <div class="mt-12 p-6 bg-gradient-to-r from-blue-500 to-purple-600 rounded-2xl text-white">
...
npx garur --watch # Watch for changes</code>
  ...
  <footer class="mt-12 pt-8 border-t border-gray-200 text-center text-gray-500 text-sm">
    <p>Made with ❤️ using Garur-CSS - The Indian CSS Framework</p>
...
async function main() {
    ...
    const args = minimist(process.argv.slice(2), {
        boolean: ['init', 'example', 'plugin', 'all'],
        alias: {
            i: 'init',
            e: 'example',
            p: 'plugin',
    ...
    if (args.h || args.help) {
        ...
        console.log(pc.gray(" garur --watch ") + "Watch mode\n");
        ...
        console.log(" garur src/**/*.tsx --watch # Build & watch\n");
        return;
    }
    // ==================== NEW: AUTO-CREATION COMMANDS ====================
    if (args.init || args.i) {
        console.log(pc.cyan("⚙️ Setting up Garur-CSS...\n"));
        createConfigFile();
    ...
    let patterns = args._.length
        ? args._
        : ["src/**/*.{html,tsx,jsx}", "open/**/*.{html}", "pages/**/*.{html,tsx,jsx}", "app/**/*.{html,tsx,jsx}", "components/**/*.{tsx,jsx}", "*.html"];
    ...
    const { loadCache } = await import("../utils/cache.js");
    ...
    let stats;
    try {
        const { runJIT } = await import("../core/jit.js");
    ...
    if (stats.cssBytes < 10_000) {
        console.log(pc.green("\nUnder 10KB — Lightning fast & production-ready!"));
    }
    else if (stats.cssBytes < 50_000) {
